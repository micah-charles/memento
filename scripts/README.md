...
The core library also exposes `ArchiveExporter.ExportRedacted(...)` for an explicitly selected set of Memory Claim IDs. It produces a portable JSONL package with Claim status, Evidence authority metadata, relationships, and attributed annotation types while withholding Source paths, transcript/audio content, provider payloads, and raw SQLite. The method rejects an empty or unknown selection and excludes withdrawn Claims by default; the shell's existing export button remains the complete authenticated archive export path.
