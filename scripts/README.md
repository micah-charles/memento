...
`Verify-MementoAutomation.ps1` is the repeatable repository verification command. It parser-checks every PowerShell helper, runs the Release test and build without restoring packages, regenerates the deterministic M04 synthetic report, and runs the read-only deployment preflight. Add `-VerifyLaunch` to include the process-level launch and graceful-close smoke; it requires no existing MEMENTO instance and does not claim native visual verification. Pass `-VerifyMsix` after building a package to include manifest, payload, artwork-dimension, and signature-policy verification; it defaults to the unsigned artifact. Supplying `-MsixPackagePath .\artifacts\msix\MEMENTO-0.1.0.0.msix` or `-RequireMsixSignature` also enables this gate, so a signed output can be verified explicitly. Use `-SkipDotnet`, `-SkipLanguageValidation`, or `-SkipPreflight` when a narrower check is needed; the optional `-RequireCloudCredential`, `-RequireApplicationLock`, `-RequireAudioInput`, `-RequireAudioOutput`, `-RequireArchiveIntegrity`, and `-RequireInstalledPayloadMatch` switches promote those deployment policies to blocking checks:
