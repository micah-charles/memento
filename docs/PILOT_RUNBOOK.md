...
- Run the read-only deployment preflight: `.\scripts\Test-MementoPreflight.ps1`; use `.\scripts\Test-MementoPreflight.ps1 -RequireCloudCredential` when the pilot includes cloud processing, and add `-RequireApplicationLock` when the family policy requires the optional app lock. Resolve every blocking check and record the warnings before continuing.
