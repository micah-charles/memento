...
**Current local checkpoint:** `fca2b52` (`test: select MSIX artifact in automation gate`).
...
- `Verify-MementoAutomation.ps1 -VerifyMsix` now delegates to the MSIX verifier and accepts `-MsixPackagePath` for signed outputs; its unsigned inspection passed, while the negative `-RequireMsixSignature` run rejected the unsigned package as expected.
