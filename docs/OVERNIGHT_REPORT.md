...
- Archive media safety — a shared reparse-point guard now protects authenticated deletion, health checks, Source reads, and derived-audio playback; the Windows regression retained a symbolic-link path and left its target untouched.
- Derived-output cleanup — post-consent-revocation and post-provider Source-policy cleanup now routes through `DerivedAudioStore.Remove`, which applies the derived-root and reparse-point guard before deleting a file or its archive row; an external-path regression keeps the target file untouched.
...
- Installed process smoke — launched via `scripts\\Start-Memento.ps1`; observed title `MEMENTO` and `Responding=True` after eight seconds, `CloseMainWindow()` returned true, and it exited cleanly. This is process evidence only; no visual GUI claim is made.
- Repeatable launch smoke — `scripts\\Test-MementoLaunch.ps1` started the installed executable with no existing MEMENTO process, observed title `MEMENTO` and `Responding=True`, requested graceful close, and observed exit code 0. This is process evidence only; it does not replace native GUI or Computer Use verification.
- Integrated local gate — `scripts\\Verify-MementoAutomation.ps1 -VerifyLaunch -RequireAudioInput -RequireAudioOutput -RequireArchiveIntegrity -RequireInstalledPayloadMatch -VerifyMsix` passed parser **13/13**, Release tests **194/194**, build **0 warnings/0 errors**, M04 **14/14**, launch/graceful-close process smoke, read-only preflight, and bundle-to-installed executable hash matching, and unsigned MSIX manifest/payload/artwork verification; the app process gate remains explicitly separate from native visual verification.
- Deployment payload integrity — `Test-MementoPreflight.ps1 -RequireInstalledPayloadMatch` compared the root executable inside the verified bundle with the installed executable and passed with SHA-256 `ccc4e16b4a46511b3ae2942ebb25cb3de932272f83f004a0713a1ba3ebece48a`; the check is warning-level by default and blocking when the switch is supplied.
- The payload hash helper uses Windows PowerShell-compatible byte conversion, and the same blocking payload-match gate passed both directly and through `Verify-MementoAutomation.ps1`'s `powershell.exe` child process.
...
- MSIX artwork replacement — `Build-MementoMsix.ps1` now copies checked-in deterministic MEMENTO artwork from `packaging/assets` instead of generating 1x1 placeholders. The latest unsigned package is 108,740,316 bytes with SHA-256 `299f683231e890985eafbe8cda732b97d67d9c4256ea5757bb09393538c23fb9`; all four artwork dimensions were inspected and `Test-MementoMsix.ps1` passed. Publisher trust and target deployment remain open.
- Final automation rerun after the MSIX dimension gate — PowerShell parser **13/13**, Release tests **194/194**, Release build **0 warnings / 0 errors**, synthetic language validation **14/14**, and deployment preflight with required audio enumeration and archive integrity all passed.
- The clean-checkout workflow command (`Verify-MementoAutomation.ps1 -SkipPreflight`) was rerun locally and passed parser **13/13**, Release tests **194/194**, Release build **0 warnings / 0 errors**, and synthetic language validation **14/14**; the committed GitHub Actions workflow leaves machine-specific deployment checks separate.
