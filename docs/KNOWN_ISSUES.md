...
| K-022 | The optional OpenAI adapters now support completed-audio transcription and a turn-based text response, but Realtime/WebRTC transport, generated response audio/TTS, and a secure client-token broker remain unimplemented. Credential Manager lookup exists for API-key use but has not been exercised with a real account. | High: a fully spoken bounded voice conversation cannot yet be claimed. | Add a reviewed realtime/TTS adapter or validate the turn-based path with explicit cloud consent on target hardware. |
