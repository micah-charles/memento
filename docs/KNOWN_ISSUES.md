...
| K-022 | The optional OpenAI adapters now support completed-audio transcription, a turn-based text response, and verified derived TTS storage, but local playback, Realtime/WebRTC transport, and a secure client-token broker remain unimplemented. Credential Manager lookup exists for API-key use but has not been exercised with a real account. | High: a fully spoken bounded voice conversation cannot yet be claimed. | Add local playback and validate the turn-based path with explicit cloud consent on target hardware; evaluate Realtime separately. |
