...
| K-022 | The optional OpenAI adapters now support completed-audio transcription, a turn-based text response, verified derived TTS storage, and an injectable NAudio WAV playback boundary, but app-level playback wiring, real output-device verification, Realtime/WebRTC transport, and a secure client-token broker remain unimplemented. Credential Manager lookup exists for API-key use but has not been exercised with a real account. | High: a fully spoken bounded voice conversation cannot yet be claimed. | Wire playback into the participant UI and validate the turn-based path with explicit cloud consent and a real output device; evaluate Realtime separately. |
