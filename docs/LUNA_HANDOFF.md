...
- Schema 18 introduces independent `CloudConversation` consent plus companion sessions, messages, text versions, chunks, spans and playback intervals. Family review can view revisions, append a correction, and play original spans.
...
2. **Harden archive and lifecycle.** Make message/turn/initial-revision writes atomic (currently separate repository calls). Preserve recognizer segment timestamps in SQLite: the parser returns them but the coordinator currently saves only whole-utterance text/spans. Verify Source+chunk registration atomicity and crash recovery of the active segment's absolute sample offset; the legacy recovery service preserves WAV bytes but does not yet reconstruct all companion chunk metadata. Add migration-from-17, backup/restore, source deletion/derived-audio cleanup and playback provenance tests. Test exactly-once playback, stop/interrupt/revoke races, startup cancellation, repeated disposal and shutdown callbacks. Retain partial streamed text on non-cancellation errors too. Confirm inputRevision/request/turn IDs survive status-only updates (COALESCE fix is present).
...
4. **Fresh deployment identity.** Enhance Publish/Verify scripts with a complete payload manifest containing source commit, App/Core DLLs, UI resources and hashes. Check publish exit code. Before recursive filesystem delete/move, verify resolved targets are within the intended publish/install directory. Commit, publish, update local installation, verify actual payload and then test via Start Menu. Current older scripts can report success matching an old bundle, so that is insufficient evidence of this checkpoint being installed.
