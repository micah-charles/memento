...
- A fresh portable publish/install was completed after the resource fix. The installed payload manifest verifies source commit `8542157`, all **609/609** file hashes and lengths, and schema 19 after a launch/close migration smoke. The next code commit (the XAML resource fix) must be republished before the installed manifest can represent it.
