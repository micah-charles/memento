...
- Windows local speech synthesis produced a Mandarin WAV using Microsoft Hanhan (`zh-TW`). The same synthetic WAV, upsampled to the capture format, was recognized by the pinned Whisper model as `你好,那个系Memento本机辨试测试。` (one segment). This proves the local adapter can run end to end on generated speech; the wording errors, CPU latency, actual microphone recognition, playback and Cantonese quality remain unmeasured.
...
- Fresh portable publish/install from `38e3d1e` created a payload manifest and installed it under `%LOCALAPPDATA%\MEMENTO\App`. Preflight verified the manifest source commit, **609/609** file hashes/lengths, Start Menu shortcut and per-user registration. SQLite integrity passed at schema version **19**.
