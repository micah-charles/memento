...
- `scripts/Verify-MementoAutomation.ps1 -VerifyMsix` now includes the MSIX manifest, payload, artwork-dimension, and optional signature gate when a package has been built; `-MsixPackagePath` selects a signed output and `-RequireMsixSignature` makes the signature requirement explicit.
