...
- Fresh install now launches and closes successfully after adding WinUI `XamlControlsResources` (the first run exposed and fixed a real `TabViewButtonBackground` XAML resource crash). Preflight verified source commit `38e3d1e`, 609/609 payload files and schema 19. Computer Use still reports `apps: []`, so native visual acceptance remains blocked; see [companion evidence](evidence/COMPANION.md).
