...
- Implemented account-backed Codex text bridge, local Whisper/Windows TTS adapters, continuous segmented capture, conversation coordinator, independent cloud consent, multi-source revision timeline and initial participant/family UI wiring. Schema 19 now preserves Whisper segment timestamps; message/turn/revision creation is atomic.
...
- Publish now emits a complete payload manifest and preflight can compare every installed file. A local Microsoft Hanhan (`zh-TW`) TTS synthesis test produced a WAV; playback/microphone/STT loop remain unverified.
- Fresh install now launches and closes successfully after adding WinUI `XamlControlsResources` (the first run exposed and fixed a real `TabViewButtonBackground` XAML resource crash). Preflight verified current source commit `1c05d4a`, 609/609 payload files and schema 19. Computer Use still reports `apps: []`, so native visual acceptance remains blocked; see [companion evidence](evidence/COMPANION.md).
- Synthetic local TTS→Whisper now executes through the pinned model and returns one timestamped segment; the recognized Mandarin sentence contains errors, so no quality or latency claim is made.
...
- Audio recovery scanning now skips reparse-point roots and descendants, and recovery refuses markers whose path contains a junction or symbolic link; the Windows negative test confirms an external marker is neither scanned nor promoted.
- PCM capture writers and derived speech stores now refuse reparse-point roots before creating date directories, so normal capture and provider-output writes cannot follow a junction outside the archive; Windows negative tests cover both writers.
...
- The cumulative suite now passes **194/194** tests. These milestones are **IMPLEMENTED / AUTOMATED TESTED**, while target-machine restart/power-loss observation, provider extraction quality, and Family Admin review remain future verification work. See [M06–M09 evidence](evidence/M06-M09.md).
...
- When a Source records a valid SHA-256 and byte length, those audio-integrity values are verified before cloud conversation work; tampered audio is rejected before provider invocation.
...
- Async memory extraction re-checks that the Source still exists after provider work completes, so a concurrent authenticated deletion cannot persist stale candidates against a removed Source.
- Extraction also refuses a stale/missing Source before invoking an async provider, preventing deleted transcript data from being sent after a caller presents an old Source object.
...
- Starting a new recording session also clears the previous session's current-information display so time-bound external answers do not cross session boundaries.
- The current-information service now normalizes provider identity and the original query at its own boundary and forcibly marks every provider result as untrusted, even if a provider returns spoofed metadata or a trusted flag.
...
- Added a Windows-only `WindowsAdministratorAuthorizer` that binds the actor ID to the current account and requires the Windows Administrators role; the fixed authorizer remains test-only and no automatic family allowlist is inferred. The WinUI shell now exposes a candidate review dialog that can select every pending claim, shows each linked Evidence statement, original expression, certainty, speaker-confirmation state, Source, and optional audio spans, then records attributed support/rejection actions.
...
- The Family Admin shell now exposes the same verified bundle re-encryption path as `更新備份密碼`: it requires the existing password and confirmation of the new value, verifies the source manifest before writing, preserves the original bundle, and records an audit operation.
...
- Windows Credential Manager reads now reject odd, empty, or oversized credential blobs before decoding; malformed application-lock blobs now fail closed instead of being treated as an absent lock, and application-lock verifier parsing also rejects oversized encoded values, keeping malformed local credential state bounded.
...
- Archive search, health checks, interrupted-audio recovery, deletion, withdrawal, and Family Admin candidate listing/annotation now run their SQLite, file, and audit work off the WinUI event thread with a shared busy-state guard, so large local archives do not freeze the participant shell or accept overlapping operations.
- The shell tracks every background archive operation and waits for them during window shutdown before disposing SQLite/runtime services, closing the lifecycle race introduced by responsive archive work.
- Added a shared archive path-safety guard. Authenticated media deletion now refuses reparse-point paths and reports the media as retained; health checks, Source reads, and derived-audio playback also fail closed instead of following a junction or symbolic link. A Windows regression confirms deletion leaves both the link and its target untouched.
- Authenticated deletion now also refuses regular media paths outside the archive root inferred from the SQLite database, so a tampered Source path cannot make Family Admin remove an unrelated user file; the regression keeps the external file and records a finding.
- Media-inclusive export and archive health checks apply the same archive-root boundary: export refuses an external regular path and cleans up its partial bundle, while health reports the path invalid without reading it as trusted archive media.
- Source playback, bounded/realtime conversation input, and derived speech reads now apply the same root boundary before opening or transmitting media, closing the remaining regular-file escape path alongside the reparse-point guard.
- Durable transcription queue processing now rejects fully-qualified Source paths outside the archive root before provider invocation, while retaining compatibility with legacy relative placeholder metadata used only by offline fixtures.
- Realtime policy-race cleanup now routes derived-output deletion through the same derived-root/reparse-point guard as reads and writes, so a tampered asset path cannot make withdrawal cleanup remove an unrelated file.
- Encrypted backup and restore now reject reparse-point input paths before reading through them; file and directory source-link regressions confirm no backup is created and the link targets remain unchanged.
- Directory backups now inspect the complete source tree before ZIP creation, rejecting nested junctions/symbolic links instead of allowing archive enumeration to follow them; restore-input symlink coverage is included as well.
- Full and scoped exports now reject reparse-point destination roots before creating any snapshot or JSONL output; the Windows regression confirms a junction target remains empty.
- The scoped redacted-export path now has its own junction regression, covering the reviewed-Claim selection flow as well as the complete export path.
...
- A publish script now produces a self-contained `artifacts/MEMENTO-win-x64.zip` plus a SHA-256 sidecar; the MSIX manifest and staging script have also been validated by generating and unpacking a single-copy unsigned package with portable Windows SDK BuildTools. A temporary development certificate produced a SignTool-verified signed package, while machine-trusted publisher identity and target install validation remain gated.
- Added `packaging/Package.appxmanifest.template.xml` and `scripts/Build-MementoMsix.ps1`. The script parameterizes publisher/version, stages the self-contained app, calls Windows SDK `makeappx`, and optionally signs with `signtool`; it checks certificate subject against the manifest publisher, supports an explicit timestamp service or an offline development signature, refuses missing tools/certificates, and labels unsigned output clearly.
- Added `scripts/Test-MementoMsix.ps1` to repeat the package unpack, manifest, single-payload, asset, and optional signature verification gates.
- `scripts/Test-MementoMsix.ps1` now validates the package identity/version, expected MEMENTO application entry, visual asset references, exact asset names, and the approved PNG dimensions before accepting the MSIX payload gate; the latest unsigned package passed this stricter check.
- The MSIX staging path now copies checked-in deterministic MEMENTO artwork from `packaging/assets` instead of generating 1x1 placeholders. The rebuilt unsigned package is **108,740,316 bytes**, SHA-256 `299f683231e890985eafbe8cda732b97d67d9c4256ea5757bb09393538c23fb9`; the artwork dimensions are `StoreLogo` 256x256, `Square150x150Logo` 150x150, `Square44x44Logo` 44x44, and `SplashScreen` 620x300. The package verifier passed; publisher certificate trust and target deployment remain open.
- `scripts/Verify-MementoAutomation.ps1 -VerifyMsix` now includes the MSIX manifest, payload, artwork-dimension, and optional signature gate when a package has been built; `-MsixPackagePath` selects a signed output and `-RequireMsixSignature` makes the signature requirement explicit, both enabling the MSIX gate automatically.
...
- Installer bundle validation now rejects traversal/rooted/duplicate-normalized/symbolic-link ZIP entries before `Expand-Archive`; a crafted traversal bundle was refused without creating an install tree or escaping the workspace, and the verified production bundle still installed successfully.
...
- The optional application lock stores a salted PBKDF2 verifier in the current Windows user's Credential Manager, blocks the participant shell and retry worker while locked, and provides configure, disable, lock-now, and unlock flows. Malformed credential reads fail closed and the UI routes configuration errors to the recovery helper instead of leaking an async exception. Pure hashing and live Credential Manager round-trip tests pass; target-machine recovery/policy review remains open.
- Added `scripts/Reset-MementoApplicationLock.ps1` as an explicit forgotten-passcode recovery path. It requires MEMENTO to be closed, accepts only the fixed current-user `MEMENTO/AppLock` target, supports `-WhatIf`, and leaves archive data untouched.
...
- The same preflight now enumerates installed NAudio Windows wave-in and active WASAPI render capabilities without opening the microphone or playing audio; this machine reports one input and one output device, while physical capture/playback, permission, and disconnect behavior remain supervised gates.
...
- `Setup-Memento.ps1` and `Test-MementoPreflight.ps1` now support optional blocking gates for enumerated input/output devices (`-RequireAudioInput` and `-RequireAudioOutput`); these gates remain distinct from physical capture/playback verification.
...
- `Verify-MementoAutomation.ps1 -VerifyLaunch -RequireAudioInput -RequireAudioOutput -RequireArchiveIntegrity -RequireInstalledPayloadMatch` now provides one complete local gate and passed parser, Release tests/build, M04 validation, launch/graceful-close process smoke, device enumeration, SQLite integrity, and bundle-to-installed executable hash matching on the installed bundle.
