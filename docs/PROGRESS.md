...
- The cumulative suite now passes **159/159** tests. These milestones are **IMPLEMENTED / AUTOMATED TESTED**, while target-machine restart/power-loss observation, provider extraction quality, and Family Admin review remain future verification work. See [M06–M09 evidence](evidence/M06-M09.md).
...
- Windows Credential Manager reads now reject odd, empty, or oversized credential blobs before decoding; application-lock verifier parsing also rejects oversized encoded values, keeping malformed local credential state bounded.
...
- The cumulative suite now passes **159/159** tests. M10–M13 are **IMPLEMENTED / AUTOMATED TESTED**, with live search, live extraction quality, supervised Family Admin UX, target-machine encrypted bundle restore, and destructive reliability drills still pending. See [M10–M13 evidence](evidence/M10-M13.md).
