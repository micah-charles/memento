...
- The cumulative suite now passes **170/170** tests. These milestones are **IMPLEMENTED / AUTOMATED TESTED**, while target-machine restart/power-loss observation, provider extraction quality, and Family Admin review remain future verification work. See [M06–M09 evidence](evidence/M06-M09.md).
...
- The cumulative suite now passes **170/170** tests. M10–M13 are **IMPLEMENTED / AUTOMATED TESTED**, with live search, live extraction quality, supervised Family Admin UX, target-machine encrypted bundle restore, and destructive reliability drills still pending. See [M10–M13 evidence](evidence/M10-M13.md).
- Added `ArchiveExporter.ExportRedacted`, a claim-ID-scoped portable export that includes selected reviewed Memory Claims, authority metadata, and linked Evidence relationships while explicitly redacting Evidence content and withholding Source IDs, transcript links, audio, provider interactions, and raw SQLite. Family Admin can now choose a reviewed Claim from the WinUI shell; the export rejects empty, unknown, withdrawn, or unreviewed selections, writes a redaction manifest, and is covered by the Release suite (now **170/170** tests).
...
- The MSIX staging path now copies checked-in deterministic MEMENTO artwork from `packaging/assets` instead of generating 1x1 placeholders. The rebuilt unsigned package is **108,730,462 bytes**, SHA-256 `8bfd5f07b067a811196938991c127f58909d739554418c10dba97587bfd5aae4`; the artwork dimensions are `StoreLogo` 256x256, `Square150x150Logo` 150x150, `Square44x44Logo` 44x44, and `SplashScreen` 620x300. The package verifier passed; publisher certificate trust and target deployment remain open.
