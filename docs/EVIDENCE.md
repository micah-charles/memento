...
- The read-only deployment preflight checks the published bundle checksum, installed executable, Start Menu shortcut, archive paths, free disk space, MEMENTO process state, and the presence of the `MEMENTO/OpenAI` credential target, while reporting native GUI/microphone/live-exchange/pilot gates as supervised verification.
