...
- In-flight transcription, response, and speech-output calls re-check Source withdrawal before persisting success metadata, derived speech, or playback; deterministic tests cover the three race boundaries.
- `dotnet test tests/Memento.Core.Tests/Memento.Core.Tests.csproj --configuration Release` passed **100/100**; full solution build passed with 0 warnings and 0 errors; the NuGet vulnerability scan reported no vulnerable packages.
...
- `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\Publish-Memento.ps1` completed successfully and produced the ignored local bundle `artifacts/MEMENTO-win-x64.zip` (106,457,539 bytes at the time of verification). The bundle is self-contained and portable; it is not a signed installer. Its SHA-256 sidecar was verified as `7620631822b6e338fbe02b1a9606fe7675c62f4a7d934e941a0135f238dffa4d`.
