...
- `dotnet test tests/Memento.Core.Tests/Memento.Core.Tests.csproj --configuration Release --no-restore` passed **169/169**; full solution build passed with 0 warnings and 0 errors; the NuGet vulnerability scan reported no vulnerable packages.
- `ArchiveExporter.ExportRedacted` was exercised with a selected Claim and linked Evidence. The output contained no SQLite snapshot or media directory, redacted statement/original-expression/annotation bodies, labelled the withheld Source link, and rejected empty or unknown selections.
...
- The latest published bundle (106,513,285 bytes; SHA-256 `2205d5fe70cf593ef8a23cfebca6fbf1141e02914a684ca532b590937b27af07`) was installed with `scripts\\Install-Memento.ps1`; its checksum preflight passed, the per-user executable and Start Menu shortcut were created, read-only preflight passed with only the optional cloud credential and application-lock warnings, and the installed executable launched successfully (`MEMENTO`, responding, 5-second smoke test). A normal `CloseMainWindow()` then exited it within ten seconds with exit code 0; the installation remains in place for the next manual GUI/microphone gate.
...
- The full Release verification after marker-boundary, extraction-transaction, clarification-chain, export-media-integrity, provider-error-redaction, streaming-backup, post-provider Source-deletion, concurrent-transcription provenance, atomic transcription/extraction retry-job deduplication, and retry-job deduplication hardening passed **167/167** tests; the rebuilt bundle was installed from its matching SHA-256 sidecar and the read-only deployment preflight passed.
