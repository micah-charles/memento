...
- `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\Publish-Memento.ps1` completed successfully and produced the ignored local bundle `artifacts/MEMENTO-win-x64.zip` (106,434,583 bytes at the time of verification). The bundle is self-contained and portable; it is not a signed installer.
