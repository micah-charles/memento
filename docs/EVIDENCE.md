...
- `dotnet test tests/Memento.Core.Tests/Memento.Core.Tests.csproj --configuration Release --no-restore` passed **156/156**; full solution build passed with 0 warnings and 0 errors; the NuGet vulnerability scan reported no vulnerable packages.
...
- `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\Publish-Memento.ps1` completed successfully and produced the ignored local bundle `artifacts/MEMENTO-win-x64.zip` (109,613,901 bytes at the time of verification). The bundle is self-contained and portable; it is not a signed installer. Its SHA-256 sidecar was verified as `5cf1bbb36bc1169f9caf2c7cc1f3e4ede6c5326f927e6f983d2f43375380d6f0`.
...
- The latest published bundle (109,613,901 bytes; SHA-256 `5cf1bbb36bc1169f9caf2c7cc1f3e4ede6c5326f927e6f983d2f43375380d6f0`) was installed with `scripts\\Install-Memento.ps1`; its checksum preflight passed, the per-user executable and Start Menu shortcut were created, and the installed executable then launched successfully (`MEMENTO`, responding, 7-second smoke test). The installation remains in place for the next manual GUI/microphone gate.
...
- The full Release verification after marker-boundary, extraction-transaction, clarification-chain, export-media-integrity, provider-error-redaction, streaming-backup, and post-provider Source-deletion hardening passed **156/156** tests; the rebuilt bundle was installed from its matching SHA-256 sidecar and the read-only deployment preflight passed.
