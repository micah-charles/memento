...
- `dotnet test tests/Memento.Core.Tests/Memento.Core.Tests.csproj --configuration Release` passed **89/89**; full solution build passed with 0 warnings and 0 errors; the NuGet vulnerability scan reported no vulnerable packages.
...
- `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\Publish-Memento.ps1` completed successfully and produced the ignored local bundle `artifacts/MEMENTO-win-x64.zip` (106,448,940 bytes at the time of verification). The bundle is self-contained and portable; it is not a signed installer. Its SHA-256 sidecar was verified as `da9a462de3d6ad319dfe1a66d2d1a612f6c2ac111a4a31c2e8b3100d5501e647`.
...
- The published bundle was installed with `scripts\\Install-Memento.ps1`; its checksum preflight passed, the per-user executable and Start Menu shortcut were created, and `scripts\\Start-Memento.ps1` then launched the installed path successfully (`MEMENTO`, responding, 5-second smoke test). The installation remains in place for the next manual GUI/microphone gate.
