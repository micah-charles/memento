...
- Deployment preflight now treats the app-local uninstaller, application-lock recovery helper, and OpenAI credential setup helper as required installed files, and enumerates both Windows wave-in and active WASAPI render devices without opening a microphone or playing audio, preventing a partial app tree from passing the deployment gate.
