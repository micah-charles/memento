...
- `AudioRecoveryService` repairs stale-but-valid capture markers, atomically promotes them to `recovered` Sources, and restores a valid marker when database registration fails; the WinUI shell exposes this as an authenticated Family Admin operation.
- New capture marker names preserve generated Session and Turn IDs for recovery provenance, while the legacy marker naming remains supported.
...
- Persisted provider and retry-worker failures now use content-free summaries (error type, safe HTTP status, or generic network/timeout category); a regression throws a message containing transcript/API-key text and confirms that neither it nor the raw message enters archive metadata.
...
- The optional `OpenAiRealtimeStreamingProvider` now streams locally-written PCM chunks through a bounded, cancellable WebSocket session, downsampling 48 kHz capture to the GA 24 kHz input shape. `RealtimeStreamingOrchestrator` rechecks live consent and Source policy, persists content-free interaction metadata, and stores successful response PCM as a verified derived WAV. Realtime response completion has a two-minute default timeout and records a content-free HTTP 504 failure when a provider never emits a terminal response, so the stop flow cannot wait indefinitely. Deterministic transport tests cover chunk ordering, response completion, timeout, consent gating, and archive persistence; no live credential or physical device call was made.
...
- `LanguageValidationDataset.Read` and the CLI `--input` path now accept an external redacted provider-result dataset, reject duplicate IDs and malformed observations, and preserve the synthetic-versus-live evidence distinction. The new reader tests pass in the cumulative Release suite.
...
- Added migrations 6–8 for retryable conversation jobs, separate Evidence/Memory Claim/link records, and person/entity/alias links; migration 14 links extraction jobs to the exact transcript revision they must process; migration 15 records authenticated deletion tombstones; migration 16 adds a rebuildable FTS5 lexical index; migration 17 repairs existing search rows so transcript hits retain their Source-to-Session provenance.
...
- Search-index health verification now checks canonical content, Source and Session provenance, duplicate IDs, and unexpected record types, so equal row counts cannot hide a corrupted FTS row.
...
- `EntityResolutionService.SuggestMatches` now offers deterministic review-only display-name/alias similarity suggestions; short names require exact matches, and suggestions never write identity or Evidence links automatically.
...
- `dotnet test tests/Memento.Core.Tests/Memento.Core.Tests.csproj --configuration Release --no-restore` passed **194/194**; full solution build passed with 0 warnings and 0 errors; the NuGet vulnerability scan reported no vulnerable packages.
- `ArchiveExporter.ExportRedacted` was exercised with a selected reviewed Claim and linked Evidence. The output contained no SQLite snapshot or media directory, redacted statement/original-expression/annotation bodies, labelled the withheld Source link, rejected empty, unknown, and unreviewed selections, and the WinUI shell now exposes the reviewed-Claim selection flow to Family Admin.
- Restore hardening now rejects destination or staging paths containing Windows reparse points before decrypting or extracting a backup; the regression test confirms a symbolic-link destination is not followed and its target remains empty.
- Atomic backup file outputs now reject reparse-point parent paths as well; the regression test confirms an encrypted file is not written through a junction target.
- Backup file and directory re-encryption now reject identical source and destination paths before opening or replacing the source; the regression confirms the original bytes remain unchanged.
- Audio recovery scanning skips reparse-point roots and descendants, and recovery rejects markers whose path contains a junction or symbolic link; the Windows regression confirms an external marker is neither scanned nor promoted.
...
- Archive media deletion now checks every referenced file path for reparse points before deleting it, while health checks, Source reads, and derived-audio reads use the same fail-closed guard. The Windows deletion regression confirms a symbolic-link media reference is retained with a finding and neither the link nor its target is removed.
- Authenticated deletion also checks that media paths stay under the archive root inferred from the database location; an external regular file is retained with a finding and the regression confirms it remains untouched.
- Media-inclusive export now rejects a regular file outside the archive root and removes its incomplete output; archive health checks count the same path as an invalid Source asset without touching the external file.
- Source playback, bounded/realtime conversation input, and derived speech reads now enforce their configured archive/derived roots before opening or transmitting a file; dedicated regressions confirm external regular files are rejected before provider or playback work.
- Realtime consent/Source policy cleanup now uses `DerivedAudioStore.Remove`, which applies the same derived-root and reparse-point guard before deleting generated output or its row; the Release suite covers successful removal and refusal of an external path.
- Encrypted backup and restore now reject reparse-point input paths before reading through them; Windows file and directory source-link regressions confirm no destination is published and the actual link targets remain unchanged.
- Directory backup validation walks every source-tree entry before ZIP creation, and restore input validation rejects a symbolic-link backup file before opening it; both regressions leave destinations and link targets unchanged.
- Full and scoped export entry points now inspect the selected destination root before creating output; a Windows junction regression confirms no export is written through the link.
- The scoped redacted-export regression exercises a valid reviewed Claim through the same boundary and confirms its junction target remains empty.
- Durable transcription jobs apply the same root check to fully-qualified persisted Source paths before invoking a provider; a regression confirms an external queue path is rejected without a provider call.
- Full/scoped exports, encrypted backup, restore verification, and backup password rotation now set the WinUI busy-state guard for their entire background operation, so repeated clicks cannot overlap archive work.
- Archive search, health checks, interrupted-audio recovery, deletion, withdrawal, and Family Admin candidate listing/annotation now run their archive and audit work off the WinUI event thread behind a busy-state guard, keeping large local operations responsive and preventing overlapping actions.
- Window shutdown now waits for the tracked background archive operations before disposing the SQLite/runtime services, so closing during export, restore, recovery, or review cannot race the archive worker.
...
- `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\Publish-Memento.ps1` completed successfully and produced the ignored local bundle `artifacts/MEMENTO-win-x64.zip` (106,512,677 bytes at the time of verification). The bundle is self-contained and portable; it is not a signed installer. Its SHA-256 sidecar was verified as `29cda6e5ad3305d8a17eb8241f310fa03df9541dd5bc8828d2776970e7b744a4`.
- `powershell -NoProfile -ExecutionPolicy Bypass -File .\\scripts\\Start-Memento.ps1` was smoke-tested against the published executable: MEMENTO remained running after 7 seconds and was then stopped cleanly. This verifies process launch only, not native-window visual content or microphone behaviour.
- The latest published bundle (109,635,441 bytes; SHA-256 `6a1f6e1df8684b42390cda1f65180c28a655e510131627cdce5c8306f7ef60af`) was installed with `scripts\\Install-Memento.ps1`; its checksum preflight passed, the per-user executable and Start Menu shortcut were refreshed, and the installed app tree now contains the multi-select scoped redacted-export shell. Native GUI and microphone gates remain supervised.
- A fresh uninstall/reinstall smoke removed the app tree, Start Menu shortcut, and current-user Installed apps registration, preserved `memory.db` byte-for-byte (SHA-256 `a9717e3827f73a9df087c098dd77cd76a51afe96d33c8b3cf41f431d46e569b1`), restored the matching bundle, and passed the required deployment preflight.
- The refreshed install was launched through `scripts\\Start-Memento.ps1`, remained responsive with window title `MEMENTO`, and closed cleanly through the normal window-close path; `Test-MementoPreflight.ps1 -RequireArchiveIntegrity -RequireAudioInput -RequireAudioOutput` then passed with SQLite integrity `ok`, one enumerated input, and one enumerated output device.
- The per-user installation now copies its uninstall script into the app tree and registers MEMENTO under the current user's Windows Installed apps registry; the registration points at the stable install path and preserves archive data by default.
...
- The installer now validates ZIP entry safety before extraction; a crafted traversal entry was rejected with no escaped output, while the sidecar-verified production bundle passed the same pre-extraction gate and installed successfully.
- Deployment preflight now treats the app-local uninstaller, application-lock recovery helper, OpenAI credential setup helper, and OpenAI credential removal helper as required installed files, and enumerates Windows wave-in input and active WASAPI render devices without opening a microphone or playing audio, preventing a partial app tree from passing the deployment gate.
- The deployment preflight now checks the `MEMENTO/OpenAI` Credential Manager target without exposing its secret; this machine correctly reports a warning when the target is absent, and `-RequireCloudCredential` turns that setup gap into a blocking failure for cloud-enabled pilots.
- The installed `Set-MementoOpenAiCredential.ps1` helper was exercised with a disposable interactive key: Credential Manager creation succeeded, the target was then deleted, and no key value was printed or persisted in the repository. This verifies setup plumbing only; no live provider request was made.
- The installed `Remove-MementoOpenAiCredential.ps1` helper was then exercised against a disposable target and confirmed `* NONE *` after deletion; MEMENTO was closed and archive data was unchanged.
- The app's normal `WM_CLOSE` path was smoke-tested after a five-second launch: the installed process reported `Responding=True`, `CloseMainWindow()` returned true, and it exited within ten seconds with exit code 0. Shutdown now waits for retry/Realtime cleanup before disposing the HTTP client and SQLite archive; this remains process evidence, not native GUI visual evidence.
- The deployment workflow can now promote read-only input/output device enumeration to blocking checks with `-RequireAudioInput` and `-RequireAudioOutput`; this does not claim a physical capture or playback test.
- The deployment preflight also checks the optional `MEMENTO/AppLock` verifier without exposing it; `-RequireApplicationLock` turns an absent verifier into a blocking failure when a family deployment policy requires the application lock.
- The read-only deployment preflight checks the published bundle checksum, installed executable, Start Menu shortcut, archive paths, free disk space, MEMENTO process state, and the presence of the `MEMENTO/OpenAI` and optional `MEMENTO/AppLock` credential targets, while reporting native GUI/microphone/live-exchange/pilot gates as supervised verification.
- Deployment preflight also verifies that the Installed apps registration points at the selected per-user install root.
...
- Restore manifest verification now hashes each file through a read stream instead of `File.ReadAllBytes`, so large audio entries do not require a whole-file memory allocation.
...
- Media-inclusive archive export now verifies snapshot-listed file presence, byte length, and valid recorded SHA-256 metadata, deleting incomplete output when verification fails; a tampered-media regression covers this failure path.
...
- Memory extraction persistence now uses an all-or-nothing SQLite batch for Evidence, candidate Claims, links, and search rows; a duplicate-claim regression confirms that a later failure leaves no partial batch.
...
- Capture start now persists `CloudTranscription` consent from the participant checkbox and selected privacy mode; an unchecked cloud control cannot become a granted consent event. The policy combinations are covered by regression tests.
- Processing now locks privacy, recording, and cloud-consent controls until the bounded cloud turn finishes, preventing an in-flight policy race in the participant shell.
...
- M04 now includes `LanguageValidationCorpus.SyntheticCases`, one non-sensitive fixture per required language category, with a coverage test that evaluates the complete corpus through the report path.
- M04 code-switch cases now declare required segments; a regression confirms that omitting a Mandarin segment produces `AcceptableWithClarification` and sets `CorrectionRequired`.
...
- Automatic extraction queue creation returns no job for private or local-only sessions, so blocked privacy modes do not leave pending cloud work behind.
- Memory extraction validates the supplied session against the persisted privacy mode before invoking a provider, covering forged in-memory session inputs.
...
- The full Release verification after marker-boundary, extraction-transaction, clarification-chain, export-media-integrity, provider-error-redaction, streaming-backup, post-provider Source-deletion, concurrent-transcription provenance, atomic transcription/extraction retry-job deduplication, and retry-job deduplication hardening passed **194/194** tests; the rebuilt bundle was installed from its matching SHA-256 sidecar and the read-only deployment preflight passed.
- `scripts/Verify-MementoAutomation.ps1` now provides a repeatable repository gate that parser-checks all PowerShell helpers, runs the Release test/build, regenerates the synthetic M04 report, and invokes the read-only deployment preflight without reading personal data or credentials.
- The deployment preflight now uses the installed native SQLite library for a read-only archive check; `-RequireArchiveIntegrity` passed with `SQLite integrity_check=ok` and schema version 17 on the installed archive.
- `dotnet list Memento.slnx package --vulnerable --include-transitive` reported no vulnerable packages for Memento.App, Memento.Core, Memento.Core.Tests, or Memento.LanguageValidation using the current NuGet source.
...
- The MSIX verifier now checks package identity/version, the expected MEMENTO application entry, visual asset references, exact asset names, and approved PNG dimensions in addition to executable shape; the latest unsigned package passed this stricter verification while signature and target deployment remain open.
- MSIX artwork is now checked in under `packaging/assets` and copied by `Build-MementoMsix.ps1`; the rebuilt unsigned package passed `Test-MementoMsix.ps1` at **108,740,316 bytes**, SHA-256 `299f683231e890985eafbe8cda732b97d67d9c4256ea5757bb09393538c23fb9`. The four PNGs are 256x256, 150x150, 44x44, and 620x300 respectively; this removes generated 1x1 placeholders without claiming publisher trust or installability.
- The repeatable automation runner now accepts `-VerifyMsix` and delegates to `Test-MementoMsix.ps1`; `-MsixPackagePath` selects a signed output and `-RequireMsixSignature` is available for a signed-package policy, with either option enabling the MSIX gate automatically. The normal runner remains usable on clean checkouts where ignored build artifacts have not yet been generated.
- `.github/workflows/windows-quality.yml` now provides a clean-checkout Windows quality job that restores the solution and runs `Verify-MementoAutomation.ps1 -SkipPreflight`; it deliberately leaves installed-app, archive, audio-device, and native GUI checks to target-machine deployment evidence.
