...
- `dotnet test tests/Memento.Core.Tests/Memento.Core.Tests.csproj --configuration Release --no-restore` passed **114/114**; full solution build passed with 0 warnings and 0 errors; the NuGet vulnerability scan reported no vulnerable packages.
...
- The full Release verification after marker-boundary, extraction-transaction, and clarification-chain hardening passed **114/114** tests; the rebuilt bundle was installed from its matching SHA-256 sidecar and the read-only deployment preflight passed.
