...
- The repeatable automation runner now accepts `-VerifyMsix` and delegates to `Test-MementoMsix.ps1`; `-RequireMsixSignature` is available for a signed-package policy. The normal runner remains usable on clean checkouts where ignored build artifacts have not yet been generated.
